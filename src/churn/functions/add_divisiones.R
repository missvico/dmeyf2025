...
# --- Reutilizamos tu division_segura (totalmente vectorizada) ---
division_segura <- function(numer, denom,
...
add_divisiones <- function(DT, mapping,
  ...
  for (out_col in names(mapping)) {
    ...
    DT[, (out_col) := division_segura(get(numer), get(denom),
                                      mode   = mode_here,
                                      on_zero= onz_here,
                                      on_na  = on_na,
                                      eps    = eps,
